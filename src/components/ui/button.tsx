...
const base =
  "group/btn relative inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full font-sans text-[0.9375rem] font-semibold tracking-normal normal-case transition-[background-color,border-color,transform,box-shadow] duration-300 ease-(--ease-out-expo) disabled:pointer-events-none disabled:opacity-50 focus-visible:outline-offset-4 [color-scheme:dark] [-webkit-text-fill-color:currentColor]";

const variants: Record<Variant, string> = {
  primary:
    "bg-ink text-cream border border-gold visited:text-cream hover:bg-stone hover:text-cream hover:border-gold active:translate-y-px",
  secondary:
    "border border-gold/70 text-gold visited:text-gold hover:border-cream hover:text-cream active:translate-y-px",
  ghost: "text-cream/85 visited:text-cream/85 hover:text-gold underline-offset-[6px] hover:underline px-0",
  dark: "bg-ink text-cream visited:text-cream hover:bg-stone active:translate-y-px",
  light: "border border-ink/25 text-ink visited:text-ink hover:bg-ink hover:text-cream active:translate-y-px [color-scheme:light]",
};
